...
**检测类型**：
1. **枚举**：语义相似度检测，识别重复的枚举定义
2. **接口**：方法签名重叠检测，识别重复的接口定义
3. **DTO**：字段结构相似度检测，识别高度相似的 DTO
4. **Options**：跨命名空间重复检测，识别重复的配置类
5. **扩展方法**：签名相同检测，识别重复的扩展方法
6. **静态类**：功能重复检测，识别重复的工具类
7. **常量**：值相同检测，识别重复的常量定义

详细的人工审查指南请参考 [代码审查检查清单 - 7类检测](./代码审查检查清单_新增7类检测.md)
