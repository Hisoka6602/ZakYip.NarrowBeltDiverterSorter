...
### 规则
项目要求统一使用本地时间，禁止使用 UTC 时间（除非在 API 边界处明确需要）。

### 允许的例外情况
如果必须使用 UTC 时间，必须在同一行添加以下精确格式的注释之一：
...
// ✅ 正确：API 边界转换
var timestamp = DateTime.UtcNow;  // 边界转换

// ✅ 正确：明确需要 UTC
var timestamp = DateTime.UtcNow;  // UTC required
```

### 注意事项
- 注释必须在同一行，不能在上一行或下一行
- 注释格式必须精确匹配 `// 边界转换` 或 `// UTC required`
- 英文注释无效，必须使用中文
- 建议：优先使用 `ILocalTimeProvider` 而非例外标记
